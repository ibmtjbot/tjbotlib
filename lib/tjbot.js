...
TJBot.prototype.defaultConfiguration = {
    ...
    speak: {
		...
		enableVoice: true // if set to false TJBot uses stdin and stdout to read and write answers using the functions 'payAttention' and 'answer'
    },
    see: {
        confidenceThreshold: {
            object: 0.5,
...
/**
 * Listen for or read the input
 */
TJBotCustom.prototype.payAttention = function (callback) {
    if(this.configuration.speak.enableVoice) {
        this.listen(function(msg) {
            callback(msg);
        });
    } else {
        this._questionAsyncRecursive(">>> ", callback);
    }
};

/**
 * Start recursive asking for console input (stdin)
 */
TJBotCustom.prototype._questionAsyncRecursive = function (question, callback) {
    rl.question(question, function (answer) {
        callback(answer);
        this._questionAsyncRecursive(question, callback);
    });
};
...
/**
 * Answer through speaker or stdout
 */
TJBotCustom.prototype.answer = function (msg) {
    if(this.configuration.speak.enableVoice) {
        let speakPromise = this.speak(msg);
		return speakPromise;
    } else {
        winston.info(this.configuration.robotname + ': ' + msg);
    }
};
 ...
 * Terminate TJBot
 */
TJBot.prototype.terminate = function () {
 ...
 */
TJBot.prototype.getContext = function (workspaceId) {
    return this._assistantContext[workspaceId];
};
 ...
 */
TJBot.prototype.setContext = function (workspaceId, context) {
    this._assistantContext[workspaceId] = context;
};
